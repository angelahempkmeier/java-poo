...
public class TelaPrincipal extends JFrame {
	...
	private FollowingComboBox followingComboBoxModel = new FollowingComboBox();
	...
	public void go() {
		FotosFeedTableModel fftm = new FotosFeedTableModel();
		tableFeed.setModel(fftm);
		tableFeed.setRowHeight(200);
		tableFeed.getColumnModel().getColumn(0).setCellRenderer(new PanelCellRenderer());
	...
	public TelaPrincipal() {
		...
		btnLoginLogin.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (!sistema.login(username, senha)) {
		...
		comboBoxBuscarPerfis.addActionListener(new ActionListener() {
			...
			if(resp == JOptionPane.YES_OPTION) {
				...
				followingComboBoxModel.addUsuarios(usuarioSelecionado);
				comboBoxSeguindo.setModel(followingComboBoxModel);
		...
		btnPostarFotoFeed.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				fileChooser.setFileSelectionMode(JFileChooser.FILES_ONLY);
				int retorno = fileChooser.showOpenDialog(null); //btnPostarFotoFeed
				if (retorno == JFileChooser.APPROVE_OPTION) {
					File file = fileChooser.getSelectedFile();
					//lblFotoPerfilFeed.setIcon(new ImageIcon(file.getPath()));
					//JLabel lblImageTable = new JLabel();
					//lblImageTable.setBounds(0,0,150, 150);
					
					ImageIcon icon = new ImageIcon(file.getPath());
					icon.setImage(icon.getImage().getScaledInstance(tableFeed.getWidth(), tableFeed.getHeight(), 1));
					p.setFoto(file.getPath());
					sistema.getUsuarioLogado().adicionarPost(p);
					
					//tableFeed.setIcon(icon);
				}  
